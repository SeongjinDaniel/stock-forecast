...
# FRED API KEY 설정
api_key = '5caf2ff9df57b265004a7835dc849e7f'
...
for code, name in fred_indicators.items():
  ...
  url = f'http://api.stlouisfed.org/fred/series/boservations'
  params = {
      'series_id': code,
      'api_key': api_key,
      'file_type': 'json',
  ...
  if response.status_code == 200:
    ...
    if data:
      ...
      df['date'] = pd.to_datetime(df['date']).dt.tz_localie(None) # tz-naive로 설정
      fred_data_frames.append(df.set_index('date'))

    else:
      print(f"No data available for indicator {name} {(code)}")
  else:
    print(f"Failed to fetch data for {name} {(code)}: {response.status_code}")
